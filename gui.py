...
class PiSliderGUI:
    ...
    def _create_holygrail_widgets(self, parent_frame):
        ...
        self.loc_frame = ttk.LabelFrame(parent_frame, text="HG Location, Ranges & Intervals", padding=5); self.loc_frame.grid(row=current_row, column=0, columnspan=4, sticky='ew', pady=(0, 10)); self.loc_frame.columnconfigure(1, weight=1)
        ttk.Label(self.loc_frame, text="Latitude:").grid(row=0, column=0, sticky='w', padx=5, pady=2); lat_entry = ttk.Entry(self.loc_frame, textvariable=self.app.holygrail_latitude_var); lat_entry.grid(row=0, column=1, sticky='ew', padx=5)
        ttk.Label(self.loc_frame, text="Longitude:").grid(row=1, column=0, sticky='w', padx=5, pady=2); lon_entry = ttk.Entry(self.loc_frame, textvariable=self.app.holygrail_longitude_var); lon_entry.grid(row=1, column=1, sticky='ew', padx=5)
        ttk.Label(self.loc_frame, text="Min Aperture (f/):").grid(row=2, column=0, sticky='w', padx=5, pady=2); min_ap_entry = ttk.Entry(self.loc_frame, textvariable=self.app.holygrail_min_aperture_var); min_ap_entry.grid(row=2, column=1, sticky='ew', padx=5)
        ttk.Label(self.loc_frame, text="Max Aperture (f/):").grid(row=3, column=0, sticky='w', padx=5, pady=2); max_ap_entry = ttk.Entry(self.loc_frame, textvariable=self.app.holygrail_max_aperture_var); max_ap_entry.grid(row=3, column=1, sticky='ew', padx=5)
        ttk.Label(self.loc_frame, text="Base ISO:").grid(row=4, column=0, sticky='w', padx=5, pady=2); base_iso_entry = ttk.Entry(self.loc_frame, textvariable=self.app.holygrail_base_iso_var); base_iso_entry.grid(row=4, column=1, sticky='ew', padx=5)
        ttk.Label(self.loc_frame, text="Max ISO:").grid(row=5, column=0, sticky='w', padx=5, pady=2); max_iso_entry = ttk.Entry(self.loc_frame, textvariable=self.app.holygrail_max_iso_var); max_iso_entry.grid(row=5, column=1, sticky='ew', padx=5)
        ...
        self.night_interval_label = ttk.Label(self.loc_frame, text="Night Interval (s):"); self.night_interval_entry = ttk.Entry(self.loc_frame, textvariable=self.app.holygrail_night_interval_var); current_row += 1
        ...
        for angle in sorted(HolyGrailController.DEFAULT_EV_TABLE.keys()):
            ...
            table_row += 1
        hg_entries = [lat_entry, lon_entry, min_ap_entry, max_ap_entry, base_iso_entry, max_iso_entry, self.day_interval_entry, self.sunset_interval_entry, self.night_interval_entry]
    ...
    def update_preview_image(self, image_path):
        ...
        try:
            # Open the image first to get its aspect ratio for default sizing
            img = Image.open(image_path)
            img_w, img_h = img.size

            # Get the current size of the container frame
            container_w = self.preview_frame.winfo_width()
            
            # <<< FIX: Use a default width of 800px if the window hasn't been drawn yet >>>
            if container_w < 100:  # A small value indicates the window isn't rendered
                target_w = 800
                # Calculate height to maintain the original image's aspect ratio
                target_h = int(target_w * (img_h / img_w))
            else:
                # Otherwise, use the actual container size for dynamic resizing
                target_w = self.preview_frame.winfo_width()
                target_h = self.preview_frame.winfo_height()

            # The thumbnail method maintains aspect ratio while fitting within the box
